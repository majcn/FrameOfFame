...
izbora. Ta obraz izrežemo iz fotografije in ga pošljemo naprej. (KAM?????)
...
Že naša prvotna izbira filtrov je izhajala iz programskega orodja za
predelavo slik GIMP. Zaradi lažjega dela se je najprej grafičnega vmesnika
izbralo zaporedje različnih filtrov z različnimi parametri
.?????????????????????????????????????????????????????????????????????????????
???????????? Izbrani so bili naslednji filtri:
\begin{itemize}
...
vzamemo samo algoritem oziroma idejo za tem in implementiramo (KOGA ALI
KAJ??????????????????????????) po naših potrebah. Naredili smo tri različne
implementacije in jih med seboj primerjali.
