...
\begin{figure}[!ht]
    ...
    \caption{15 sekund slave slike umetnika Andy Warhol~\cite{katalog_solina}}
...
prikazano kot zelen pravokotnik okrog obraza na
sliki~\ref{fig:lena_opencv_face}; določitev regije, kjer je potrebno iskati;
detekcija oči v določeni regiji, prikazano kot dva modra kroga okrog oči
...
v~\refPoglavju{ch:ohranjanjeDigitalneUmetnosti}, da je težko nadgraditi le eno
komponento, saj obstaja možnost, da ni kompatibilna z ostalo opremo. Ker stari
...
prenos slike iz telefona na zaslon uspešen.
...
popart slika lahko kmalu javno objavljena na enem izmed socialnih omrežij, kar
lahko naredi tako,  da se na primer dotakne ikone in tako odloči, da se njegov
portret prenese na želeno socialno omrežje.
...
\section{Implementacija z socialnim omrežjem Facebook}
...
API-ja potrebno narediti novo aplikacijo (angl. app). Tu določimo, katere
pravice želimo, da naša aplikacija uporablja. Več pravic, ki jih bomo
zahtevali, manj zaupljiva bo naša aplikacija. Zato je priporočljivo, da se
zahtevajo le pravice, ki jih nujno potrebujemo za delovanje naše aplikacije. V
našem primeru potrebujemo pravico, da lahko pišemo in objavljamo slike na zid
uporabnika.

Ko pripravimo našo aplikacijo, je tudi potrebno, da jo uslužbenci pri podjetju
Facebook potrdijo. To je varnostni postopek, brez katerega naše aplikacije ne
moremo javno uporabljati.

Ko imamo narejeno in potrjeno aplikacijo, se lahko preko nje povežemo na Graph
API. Od tukaj naprej imamo več možnosti, a za nas primerne le dve. Prva
možnost je najbolj splošna, in sicer da uporabljamo kar zahteve preko HTTP
protokola. A ker je naša celotna instalacija napisana za operacijski sistem
Android, lahko namesto tega uporabimo Facebook Android SDK.

S pomočjo Facebook Android SDK je sama integracija na operacijskem sistemu
Android enostavnejša, saj vsa komunikacija z Graph API-jem poteka preko
