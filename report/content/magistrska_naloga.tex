...
umetniško instalacijo ``15 sekund slave''. Najprej se bomo v \ref{ch:AndyWarhol_popart}.~poglavju
seznanili z motivacijo za to instalacijo in spoznali, kako instalacija deluje.

V \ref{ch:prvotnaIzvedba15}.~poglavju se seznanimo, kako je bila instalacija prvotno implementirana.

Da bi se naloge pravilno lotili, smo v \ref{ch:ohranjanjeDigitalneUmetnosti}.~poglavju preučili širšo problematiko
vzdrževanja novomedijskih umetniških del. Kako sploh vzdrževati umetniško instalacijo, ki temelji na računalniški
...
V \ref{ch:nadgradnjaNapravaPripomocki}.~poglavju smo po preučevanju teorije, kako se sploh pravilno lotiti
...
operacijski sistem Android~(\ref{sec:android}). Po izbiri strojne opremo smo začeli z načrtovanjem, kako prenesti vso
obstoječo programsko funkcionalnost.

V \ref{ch:zaznavanjeObrazov}.~poglavju smo se soočili z zaznavanjem obrazov.
...
V \ref{ch:obdelavaSlik}.~poglavju smo obravnavali obdelavo slik. Že pri
...
Vendar pa smo želeli še več. V \ref{ch:novosti}.~poglavju opisujemo nove
funkcionalne možnosti, ki smo jih dodali za popestritev instalacije.

V \ref{ch:socialNetwork}.~poglavju pa smo se ukvarjali z najbolj pomembno lastnostjo, ki je pravi
hit sedanjega časa in sicer s povezavo s socialnimi
omrežji. Mladim in vseskozi tudi starejšim postaja
