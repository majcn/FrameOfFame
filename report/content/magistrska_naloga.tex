...
je leta 1964 pri razstavi Ameriški Supermarket le posrečilo. Ta razstava je bila
ena izmed prvih, ki je javnost seznanila s pop artom. Nato se je njegova kariera le še vzpenjala.
...
Samsnug Galaxy S5 se ponaša s štiri-jedrnim procesorjem 2.5GHz Krait 400 na čipu Qualcomm MSM8974AC Snapdragon 801. Ta procesor je dovolj zmogljiv za preračunavanje vseh filtrov v zadovoljivem času.
...
Začetek projekta sega v leto 1995, kjer se je vse skupaj začelo kot semestrski projekt na Univerzi
Kalifornije, Berkeley. Avtorja Spencer Kimball in Peter Mattis sta
projekt poimenovala \textit{angl. General Image Manipulation Program}, čez nekaj časa pa sta si premislila in Richarda Stallmana, ki je ravno takrat obiskal univerzo, prosila, če lahko
spremenita besedo ``General'' v ``GNU''. Od takrat naprej se program imenuje
\textit{angl. GNU Image Manipulation Program} ali na kratko še vedno
GIMP~\cite{wiki:GIMP}.


\section{Izbira filtrov}
Že naša prvotna izbira filtrov je izhajala iz programskega orodja za predelavo slik
...
bolj kompleksni, kot bi bilo potrebno za naš projekt. Zato smo se odločili, da vzamemo samo
algoritem oziroma idejo za tem in implementiramo (KOGA ALI KAJ??????????????????????????) po naših potrebah. Naredili
...
v programskem jeziku C. Rezultati so pokazali, da je ta rešitev nekoliko počasnejša kot prejšnja, vendar pa za to nismo potrebovali grafične kartice na
...
Po izračunanih vrednostih dane rezultate iz RGB prostora prenesemo v HSL prostor
...
svetlost. $H$, $S$ in novo pridobljeni $L$ prenesemo nazaj v RGB prostor.
...
Dan danes je to medij, preko katerega se srečamo z veliko ustvarjalnimi idejami.
...
okrašen, le na sredini bo združen, kot je prikazano na
...
lahko kmalu javno objavljena na enem izmed socialnih omrežij.
...
In če se podamo še v najhujšo skrajnost, nekdo bi lahko napisal program, ki vse slike shranjuje
na nek strežnik in nato z njimi razpolagal po svoji volji.
